#!/usr/bin/env python3
"""
query_ogs_events_automatic.py

Automated Oklahoma Geological Survey (OGS) Earthquake Event Query & Exporter.

Fetches earthquake events from the USGS / OGS FDSN WS API for Oklahoma,
filters events matching the `ogs****????` pattern (e.g., ogs2024ahys),
and exports the dataset into CSV format matching wichita_ogs_ou_edu_archive/complete.csv.

Features for automated/production deployment:
- Atomic file writes via temporary files to prevent race conditions during web access.
- Flexible execution modes: single timeframe or `--all` mode to update 1-day, 7-day, and 30-day datasets in one run.
- Ensures the oldest event receives objectid = 1, incrementing chronologically up to N for the newest event.
"""
...
def query_ogs_events(days=7, start_str=None, end_str=None, min_mag=None, max_mag=None, output_file="ogs_events_past_7days.csv", quiet=False):
    ...
    Uses atomic file replacement (.tmp -> final .csv) to ensure thread/process safety.
    """
    if not end_str:
    ...
    if not quiet:
        print(f"[*] Querying OGS event API ({days} days): {query_url}")
    ...
    # Write output atomically using a temporary file first
    ...
    if not quiet:
        print(f"[+] Successfully exported {len(event_rows)} events atomically to '{output_path}'.")

    return output_path


def run_all_updates(quiet=False):
    """
    Runs automated updates for 1-day, 7-day, and 30-day datasets,
    and automatically merges all queried events into ogs_events_all_complete.csv.
    """
    tasks = [
        (1, "ogs_events_past_1days.csv"),
        (7, "ogs_events_past_7days.csv"),
        (30, "ogs_events_past_30days.csv"),
    ]
    generated_files = []
    for days, filename in tasks:
        if not quiet:
            print(f"\n=== Automated Update: Past {days} Day(s) -> {filename} ===")
        out_path = query_ogs_events(days=days, output_file=filename, quiet=quiet)
        generated_files.append(out_path)

    # Automatically merge into master ogs_events_all_complete.csv
    if not quiet:
        print("\n=== Automatically Merging Events into Master Catalog (ogs_events_all_complete.csv) ===")
    from merge_ogs_events import merge_events
    merge_events(input_files=generated_files, quiet=quiet)


def main():
    parser = argparse.ArgumentParser(
        description="Automated query script for OGS earthquake events with atomic CSV exports."
    )
    parser.add_argument(
        "--all",
        "-a",
        action="store_true",
        help="Update all standard CSV datasets (1-day, 7-day, and 30-day) and merge into ogs_events_all_complete.csv.",
    )
    parser.add_argument(
        "--days",
        "-d",
        type=int,
        default=7,
        help="Number of past days to query (default: 7). Ignored if --all is set.",
    )
    parser.add_argument(
        "--start",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Output CSV filename (default: ogs_events_past_<N>days.csv).",
    )
    parser.add_argument(
        "--no-merge",
        action="store_true",
        help="Skip merging output into ogs_events_all_complete.csv.",
    )
    parser.add_argument(
        "--quiet",
        "-q",
    ...
    if args.all:
        run_all_updates(quiet=args.quiet)
    else:
        out_file = args.output if args.output else f"ogs_events_past_{args.days}days.csv"
        out_path = query_ogs_events(
            days=args.days,
        ...
        if not args.no_merge:
            if not args.quiet:
                print("\n=== Merging queried events into Master Catalog (ogs_events_all_complete.csv) ===")
            from merge_ogs_events import merge_events
            merge_events(input_files=[out_path], quiet=args.quiet)
